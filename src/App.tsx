...
async function sbPatch(invoiceId, patch) {
  const res = await fetch(
    `${CONFIG.url}/rest/v1/${CONFIG.table}?invoice_id=eq.${encodeURIComponent(
      invoiceId
    )}`,
    {
      method: 'PATCH',
...
async function sbTimeline(invoiceId) {
  try {
    const res = await fetch(
      `${
        CONFIG.url
      }/rest/v1/delivery_timeline?select=*&invoice_id=eq.${encodeURIComponent(
        invoiceId
      )}`,
      { headers: HDRS() }
    );
...
/* ══════════════════════════════════════════════════════════════════════
   LOGIN CREDENTIALS  ── har store ka username + password. Head = admin.
   ══════════════════════════════════════════════════════════════════════ */
const STORE_CREDENTIALS = {
  MOH: { username: 'mohali_hjs', password: '1111' },
  CHD: { username: 'chandigarh_hjs', password: '1112' },
  GGN: { username: 'gurgaon_hjs', password: '1113' },
  NCR: { username: 'delhi_hjs', password: '1114' },
  NOD: { username: 'noida_hjs', password: '1115' },
  LDH: { username: 'ludhiana_hjs', password: '1116' },
  JAL: { username: 'jalandhar_hjs', password: '1117' },
  JPR: { username: 'jaipur_hjs', password: '1118' },
  LKO: { username: 'lucknow_hjs', password: '1119' },
  NWD: { username: 'newdelhi_hjs', password: '1120' },
  JKP: { username: 'janakpuri_hjs', password: '1121' },
};
const HEAD_CREDENTIALS = { username: 'admin_hjs', password: 'admin@123' };
...
function checkLogin(username, password) {
  ...
  if (u === HEAD_CREDENTIALS.username && p === HEAD_CREDENTIALS.password) {
    return {
      branch: 'ALL',
      isHead: true,
      name: 'Head Office',
  ...
  const hit = Object.entries(STORE_CREDENTIALS).find(
  ...
  if (hit)
    return {
      branch: hit[0],
      isHead: false,
      name: branchLabel(hit[0]),
      storeName: branchLabel(hit[0]),
    };
  return null;
}
...
/* app-controlled timeline: each move/edit logs an event with the fields entered
   NOTE (fix): "talked" stage no longer carries Date/Time — those belong to
   "scheduled" only. Talked-to-customer just logs Remarks if any were entered. */
...
function Login({ onLogin }) {
  ...
  const go = () => {
    const res = checkLogin(username, pw);
    if (!res) {
        ...
        <div className="glass-card">
          <div style={{ marginBottom: 6 }}>
            ...
            <div style={{ fontSize: 13.5, color: T.inkSoft, marginTop: 4 }}>
              Username aur password se sign in karo.
            </div>
          </div>
          <Field label="Username">
            <input
              className="inp"
              placeholder="mohali_hjs"
              value={username}
              onChange={(e) => {
          ...
          >
            ...
            <br />
            Store: <b>&lt;city&gt;_hjs</b> / 1111… &nbsp;·&nbsp; Head:{' '}
            <b>admin_hjs</b> / admin@123
          </div>
        </div>
      </div>
    </div>
...
function Drawer({ d, onClose, onAdvance, onSetStage, onEditStage }) {
  ...
  // FIX: "talked" block removed — it had no field of its own and was just
  // duplicating "scheduled"'s Date/Time. Timeline now starts at Scheduled.
  const blocks = [
    {
      id: 'scheduled',
      i: 1,
    ...
    },
    {
      ...
      rows: [
        ...
        [
          'Amount',
          r.amount_collected
            ? `₹${Number(r.amount_collected).toLocaleString('en-IN')} · ${show(
                r.amount_type
              )}`
            : '—',
        ],
        [
          ...
          r.security_collected
            ? `₹${Number(r.security_collected).toLocaleString(
                'en-IN'
              )} · ${show(r.security_type)}`
            : '—',
        ],
  ...
  return (
    <style>{`
      ...
      * { box-sizing: border-box; }
      ...
      .tl-field { font-size: 12.5px; color: ${T.inkSoft}; margin-top: 3px; font-weight: 500; }
